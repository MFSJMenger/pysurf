...
class RunTrajectory(Colt):
    ...
    initial condition = initcond.db :: str                                                           
    ...
    def __init__(self, inputfile):
        ...
        if exists_and_isfile(self.config['initial condition']):
            self.initcond = InitialConditions.from_db(self.config['initial condition'])
        else:
            self.logger.error('Database with the initial condition not found')
